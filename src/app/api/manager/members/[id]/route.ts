...
import { updateMember, deactivateMember } from "@/lib/server-actions/member-crud";
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  const r = await deactivateMember({ tenantId: ctx.tenantId, memberId: params.id, prisma });
  if (!r.success) return NextResponse.json({ error: r.error }, { status: 400 });
  return NextResponse.json({ ok: true });
}
